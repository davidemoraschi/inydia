...
INPUTERROR:
    ...
    '**************************************************************************
    '* davide 5/11/2010: deshabilita la dialog box de parámetros al principio *
    '*------------------------------------------------------------------------*
    '**************************************************************************
    'Dialog Dlg
    '**************************************************************************
...
ProcessLayout = "TEST"
    ...
    '***********************************************************************************
    '* davide 01/12/2010: nombre del fichero de configuración con los parámetros       *
    '*---------------------------------------------------------------------------------*
    '* al principio de la macro lee el fichero INI_FILE_NAME con los valores de la     *
    '* personalización del proceso. Si no lo encuentra pregunta al usuario.            *
    '* 		1. Test Setup															   *
	'* 		2. Vol quota															   *
    '* 		3. Vol mínimo															   *
    '* 		4. N. puntas															   *
    '* 		5. Nombre del Sample rack (prefijo)										   *
    '* 		6. Nombre del rack destino  (prefijo)									   *
    '* 		7. Num placa destino													   *
    '* 		8. Ubicación reportes													   *
    '* 		9. Ubicación resultados													   *
    '* 		10. Número de muestras													   *
    '* 		11. Utilizar cuotas incompletas (true/false)							   *
    '***********************************************************************************
	'START CODING HERE -----------------------------------------------------------------

	Const INI_FILE_NAME = MacroDir$ & "\ejecuta.ini"
	Debug.Print "Reading INI file: ";
    Debug.Print INI_FILE_NAME
...
'MacroSetting End ++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ...
    Dim NumberOfTips As Integer '= 8
    Dim ProcessSourceTubes As Integer  '= 24
	Dim VolOfQuota As Long  '= 500
    Dim IncompleteQuota As Boolean  '= True 	' ***** Propiedad de la clase INyDIA_SourceTube
    Dim MinLastQuota As Long '= 100 			' ***** Propiedad de la clase INyDIA_SourceTube
    Dim SourceRackPref As String
    Dim DestRackPref As String
    Dim DestPlateNum As Long
    Dim DestReptFold As String
    Dim DestResFold As String


	If Exists_FileINI(INI_FILE_NAME) Then
		ProcessLayout 	= Lee_Fichero_INI_str(INI_FILE_NAME, 	"ProcessLayout")
		VolOfQuota 		= CLng(Lee_Fichero_INI_lng(INI_FILE_NAME, 	"VolOfQuota"))
		MinLastQuota 	= CLng(Lee_Fichero_INI_lng(INI_FILE_NAME, 	"MinLastQuota"))
		NumberOfTips 	= CInt(Lee_Fichero_INI_int(INI_FILE_NAME, 	"NumberOfTips"))
		SourceRackPref 	= Lee_Fichero_INI_str(INI_FILE_NAME, 	"SourceRackPref")
		DestRackPref 	= Lee_Fichero_INI_str(INI_FILE_NAME, 	"DestRackPref")
		DestPlateNum 	= CLng(Lee_Fichero_INI_lng(INI_FILE_NAME, 	"DestPlateNum"))
		DestReptFold 	= Lee_Fichero_INI_str(INI_FILE_NAME, 	"DestReptFold")
		DestResFold 	= Lee_Fichero_INI_str(INI_FILE_NAME, 	"DestResFold")
		ProcessSourceTubes 	= CInt(Lee_Fichero_INI_int(INI_FILE_NAME, 	"ProcessSourceTubes"))
		IncompleteQuota 	= CBool(Lee_Fichero_INI_bool(INI_FILE_NAME, 	"IncompleteQuota"))
	Else
		Err.Raise(-1571, "y-Plat Aliquot", "No se encuentra el fichero " & INI_FILE_NAME)
		'& " u el fichero es demasiado antiguo.")
	End If


	xs.WriteLog(LOG_SUBDET, "Lectura códigos de placas")
	Debug.Print "Lectura códigos de placas"

    '***********************************************************************************
    '* davide 5/11/2010: proceso de lectura código de barras de la(s) placa(s) destino *
    '*---------------------------------------------------------------------------------*
    '* genera un fichero muestras.dat con el número de muestras, un plateBC.dat con el *
    '* código de barras de 1 o 2 placas de destino                                     *
    '***********************************************************************************
	'START CODING HERE -----------------------------------------------------------------
	...
	xs.WriteLog(LOG_SUBDET, "Lectura códigos de barras de las muestras")
	Debug.Print "Lectura códigos de barras de las muestras"

    '***********************************************************************************
    '* davide 5/11/2010: proceso de lectura código de barras de los tubos origen       *
    '*---------------------------------------------------------------------------------*
    '* genera un fichero bcdata.dat con los códigos de barras de los tubos muestras    *
    '***********************************************************************************
	'START CODING HERE -----------------------------------------------------------------
	'Variable declaration
	'SUBPROCESS_ACTIVE = True
    ...
    n = 1
'	DestRackName(0) = "MP_001" '<-DestRackPref
'	DestRackName(1) = "MP_002" '<-DestRackPref
	...
	If Exists_File(fchbcdata) Then 'Hay que comprobar que el fichero no sea antiguo
		If vNumMuestras <> Lee_Numero_de_Muestras_enbcdata(fchbcdata) Then
			Err.Raise(-1572, "y-Plat Aliquot", "El número de códigos de barra en " & fchbcdata & " no coincide con el nímero en " & fchMuestras)
		End If
	Else
	...
	'CurrentTubeSet.SourceRackName = "SampleRack_001" '<-SourceRackPref
	...
	For i = 1 To vNumMuestras
		...
		Call InsertRecord(vExecution_ID, BCNumber, BCRack, BCPos, DestRackName(CurrentRack), aNumPlacas(CurrentRack), CurrentPos, WellMatrix(CurrentPos), CurrentQuota, db)
...
'		xs.WriteLog(LOG_DEBUG, "Líquidos detectados: " & CStr(xvol.Vol(0)) & " - " & CStr(xvol.Vol(1)) &  " - " & CStr(xvol.Vol(2)) &  " - " & CStr(xvol.Vol(3)))
'
'		For a = 0 To 7
...
'
'						Call InsertRecord(vExecution_ID, BCNumber, CurrentTubeSet.SourceRackName, i + .TubeOrderNo, DestRackName(CurrentRack), aNumPlacas(CurrentRack), CurrentPos, WellMatrix(CurrentPos), CurrentQuota, db)
...
'
	...
	xs.WriteLog(LOG_SUBDET, "Distribución de las muestras")
	Debug.Print "Distribución de las muestras"

    '***********************************************************************************
    '* davide 5/11/2010: proceso de lectura distribución del liquido                   *
    '*---------------------------------------------------------------------------------*
    '* aquí se lanza la macro para distribuir las muestras 1 a 1                       *
    '*---------------------------------------------------------------------------------*
    '***********************************************************************************
...
'Next i
	...
	xs.WriteLog(LOG_SUBDET, "Creación del informe")
	Debug.Print "Creación del informe"
	...
	xs.WriteLog(LOG_SUBDET, "Terminando proceso")
	Debug.Print "Terminando proceso"
...
Sub InsertRecord(vExecution_ID As Long, vBarCode As String, vSourceRack As String, vSourceTube As Integer, vTargetRack As String, vTargetRack_ID As Long, vWellNumber As Integer, vPosition As String, vCurrentQuota As Long, db As DAO.Database)
...
Sub FillWellMatrix96pos(WellMatrix() As String)
      ...
      WellMatrix(0)="!!" 'No existe en la placa la posición 0
...
Dim conn As Object, rts As Object
	...
	Call rts.open("SELECT [TargetRack] AS [Nombre placa destino], [TargetRack_ID] AS [Identificación placa destino],  [Position] AS [Posición destino], [BarCode] AS [Identificación muestra] FROM qry_ExportHTML WHERE [Execution_ID]= " & vExecution_ID, conn)
...
Function Insert_Execution(strFilePath As String) As Long
	...
	rs!Execution_Notes = "y-Plat Aliq: distribución 1 a 1"
...
Dim codPlaca As String
	...
	If codPlaca <> "" Then
		Lee_Codigos_de_Placas = CLng(codPlaca)
	Else
		Lee_Codigos_de_Placas = CLng(0)
	End If
...
'Dim ff
	...
	'Set ff = Nothing
End Function
Function Lee_Fichero_INI_str(fchINI As String, strVariable As String) As String
...
End Function
Function Lee_Fichero_INI_lng(fchINI As String, strVariable As String) As Long
Dim str_Result As String
	...
	If str_Result = "" Then
		Err.Raise(-1572, "y-Plat Aliquot", "Valor erróneo u inexistente para " & strVariable)
	Else
    	On Error GoTo X
			Lee_Fichero_INI_lng = CLng(str_Result)
		On Error GoTo 0
	End If
	Exit Function
...
End Function
Function Lee_Fichero_INI_int(fchINI As String, strVariable As String) As String
	Lee_Fichero_INI_int = ff.GetINIString(fchINI, "INyDIA", strVariable)
End Function
Function Lee_Fichero_INI_bool(fchINI As String, strVariable As String) As String
	Lee_Fichero_INI_bool = ff.GetINIString(fchINI, "INyDIA", strVariable)
End Function
